...
output_file_name <- paste("./results/sub-models/daily_forecasts/er_lags20_tau", tau, "_forecasts.txt", sep="")
...
temp_data <- df_avg_temp_fixed_fill %>% select(meter_id, date_only, avg_temp) %>% rename(date=date_only, temp=avg_temp) %>%
  group_by(meter_id) %>% mutate(min=min(temp), max=max(temp)) %>%ungroup()%>% mutate(temp=(temp-min)/(max-min)) %>% select(meter_id, date, temp)
temp_data_new <- temp_data
temp_data_new$date <- rep(seq(from=as.Date("2018/01/01"), to=as.Date("2018/12/31"),by = "day"), 3248)
temp_data_new <- append_row(temp_data_new%>%as_tsibble(index=date, key=meter_id), (14)) %>% fill(temp)
temp_data <- rbind.data.frame(temp_data, temp_data_new)
...
for (i in 1:horizon){
  ...
  test_data <- test_data %>% select(-day_of_month, -day_of_week, -month_of_year)
  test_data <- test_data %>% mutate(date=date+1) %>% left_join(metadata, by="date")
  test_data <- test_data %>% select(-temp)
